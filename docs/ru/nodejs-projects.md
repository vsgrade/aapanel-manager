...
> 🔧 Остальные вкладки окна «Изменить» (**Маппинг, Перезапись URL, Конфигурация, SSL, Нагрузка, Статус сервиса, Модуль**) при открытии **отдельный GET-запрос не шлют** — данные берутся из [`get_project_info`](#2-get_project_info), а свои эндпоинты эти вкладки раскрывают только при действии (применить SSL, сохранить правило и т.п.). Снять рецептом «разведка → исполнение» при выполнении действия.
