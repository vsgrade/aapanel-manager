...
| 10 | [Логи](#10-логи) | Лог проекта (PM2/сборка) и лог сайта (nginx) |
| 11 | [Модули](#11-модули) | Список модулей + установка зависимостей в 1 клик |
| 12 | [SSL](#12-ssl) | Статус SSL + выпуск Let's Encrypt |
...
> ✅ Вживую подтверждены **все четыре**: `start`, `stop`, `restart`, `delete` (одинаковый формат тела). В UI операция «Возобновление проекта» = `restart`, «Остановить проект» = `stop`, «Запуск проекта» = `start`, «Удалить сайт» = `delete`.
...
Вкладки «Лог проекта» и «Лог сайта» в окне «Изменить». Оба метода реально сняты; ответ — текст лога в `message.result`.

**Лог проекта (вывод PM2 / сборки) — `POST /v2/project/nodejs/get_project_log`**
Тело: `data={"project_name":"myapp"}`
```json
{ "status": 0, "message": { "result": "<текст лога: вывод запуска/сборки PM2>" } }
```

**Лог сайта (access/error nginx) — `POST /v2/site?action=GetSiteLogs`**
> ⚠️ Это **общий** эндпоинт сайтов (`/v2/site`), не Node-специфичный. Параметр — **плоское** поле `siteName`, без обёртки `data=`.
Тело: `siteName=myapp`
```json
{ "status": 0, "message": { "result": "<текст nginx-лога>" } }
```

> ⚠️ Ответы логов **большие** и содержат реальные данные (IP, домены, пути) — при документировании/логировании обезличивайте.

---

## 11. Модули

Вкладка «Модуль» в окне «Изменить» — npm-зависимости проекта.

**Список модулей — `POST /v2/project/nodejs/get_project_modules`**
Тело: `data={"project_name":"myapp","project_cwd":"/www/node-projects/myapp"}`
```json
{ "status": 0, "message": [ /* установленные модули; [] если ещё не установлены */ ] }
```

**Установка зависимостей в 1 клик — `POST /v2/project/nodejs/install_packages`**
> Кнопка «Установка в 1-клик» ставит **все** зависимости из `package.json` проекта (`npm/pnpm/yarn install`).
Тело: `data={"project_name":"myapp"}`
*(запрос долгий — установка идёт в фоне; статус смотрите через `get_project_modules`)*

---

## 12. SSL

Вкладка «SSL» в окне «Изменить». Подвкладки: **Текущие сертификаты, Коммерческий сертификат, Let's Encrypt, Владелец сертификата**.

**Статус SSL — `POST /v2/site?action=GetSSL`**
> ⚠️ Общий эндпоинт сайтов (`/v2/site`), плоское поле `siteName`.
Тело: `siteName=myapp`
```json
{ "status": 0, "message": {
...
```
(`status:false` — сертификат не установлен.)

**Проверка домена для Let's Encrypt — `POST /v2/ssl_domain?action=check_domain_automatic`**
> Перед выпуском панель проверяет, можно ли валидировать домен. Плоское поле `domain`.
Тело: `domain=myapp.example.com`
```json
{ "status": 0, "message": { "hash": "", "domain": "myapp.example.com", "support": [] } }
```
| Поле | Что значит |
|------|-----------|
| `support` | доступные способы проверки; **пустой массив = домен не проходит** (нет A-записи на сервер) → выпуск не состоится |

> ⚠️ **Сам запрос выпуска Let's Encrypt не снят:** на тест-сервере у домена нет A-записи, проверка `check_domain_automatic` вернула пустой `support`, и панель до запроса выпуска не доходит. Чтобы снять выпуск — нужен домен с A-записью на IP сервера; тогда после успешной проверки сработает основной запрос выпуска (снять рецептом «разведка → исполнение»).

---

## 13. Прочие вкладки окна «Изменить»

Каждую вкладку проверял **по отдельности** — вот что реально уходит при открытии:

| Вкладка | Запрос при открытии |
|---------|---------------------|
| **Конфигурация** (vhost nginx) | `POST /v2/files?action=GetFileBody`, тело `path=/www/server/panel/vhost/nginx/node_<проект>.conf` |
| **Перезапись URL** (rewrite) | `POST /v2/files?action=GetFileBody`, тело `path=/www/server/panel/vhost/rewrite/node_<проект>.conf` |
| **Нагрузка** (CPU/RAM) | `get_project_info` (поле `load_info`) |
| **Статус сервиса** | `get_project_info` (поля `run`, `listen`, `listen_ok`) |
| **Маппинг** (reverse-proxy) | при открытии запрос **не шлёт** — список из `get_project_info`; свой эндпоинт — только при добавлении маппинга |

> 💡 **Конфигурация и Перезапись URL — это просто редактирование файла** через общий files-API: читается `GetFileBody`, сохраняется парным **`SaveFileBody`** (тело: `path=<файл>&data=<содержимое>&encoding=utf-8`). То есть «изменить конфиг nginx / правила rewrite по API» = записать соответствующий файл.
