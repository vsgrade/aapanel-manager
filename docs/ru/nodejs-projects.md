...
> 🔧 Оставшиеся вкладки окна «Изменить» (**Маппинг, Перезапись URL, Конфигурация, Нагрузка, Статус сервиса**) при открытии GET не шлют — данные из [`get_project_info`](#2-get_project_info); их эндпоинты появляются при действии (сохранить правило и т.п.).
