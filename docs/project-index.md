...
| `web/src/server/actions/updates.ts` (+`lib/validation/update-settings.ts`) | Раздел «Версии/обновления»: статус, чтение/сохранение настроек, **`stageUpdateAction`** (2a: скачать+проверить+бэкап+миграции), **`activateUpdateAction`/`rollbackUpdateAction`** (2b: своп симлинка + рестарт своего проекта через aaPanel API) — admin+аудит |
| `web/src/lib/deploy/*` | Самообновление панели: `adapter` (интерфейс `DeployAdapter`: preflight/stage/**activate/rollback**), `aapanel` (реализация; `swapAndRestart` — атомарный своп `current` + рестарт), `layout` (пути релизов + защита версии от path-traversal), `bundle-assets`/`bundle` (поиск+sha256 / IO: скачать/проверить/`tar`), `db-backup` (`pg_dump` через env), `migrate` (`prisma migrate deploy` из бандла), `index` (фабрика) |
| `web/src/app/api/health/route.ts` + `src/proxy.ts` | Публичная liveness/version-проба `{ok,version,...}` (для проверки «новая версия поднялась» + надсмотрщиков); `proxy` исключает `/api/health` из auth-гейта |
| `web/src/app/(app)/settings/` + `components/settings/*` | Страница «Настройки» (RSC, admin-only): `update-status-card` (статус/changelog/команда/история), **`update-actions`** (кнопки Подготовить→Применить/Откатить + подтверждение + поллинг `/api/health`), `update-settings-form` (способ установки + источник) |
...
| `web/prisma/schema.prisma` | Модели: User/Server/ServerStatus/AuditLog + **UpdateSettings** (singleton: способ установки, GitHub-репо, зашифр. токен, **stagedVersion/stagedAt/previousVersion**) + **VersionHistory** (история версий) |
