...
- **UI** в «Настройки → Версии»: кнопки «Подготовить {v}» → «Применить» → «Откатить на {v}» (подтверждение + поллинг `/api/health` после рестарта), i18n ru/en.
...
2. В панели **Настройки → Версии**: способ установки = **aaPanel**, выбрать сервер (тот, что хостит панель) и имя её Node-проекта; указать GitHub `owner/repo`.
