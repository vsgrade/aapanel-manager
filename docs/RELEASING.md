...
- Серверная «начинка»: скачивание, **проверка sha256**, атомарная распаковка, бэкап БД, миграции (`web/src/lib/deploy/`), действие `stageUpdateAction`, `/api/health`.

Осталось (Фаза 2b):
- Кнопки «Подготовить → Применить» / «Откатить» в админке через `DeployAdapter.activate()/rollback()` (переключение симлинка `current` + перезапуск своего Node-проекта через aaPanel API + health-поллинг).
- Дока одноразовой настройки (раскладка `releases/<v>` + симлинк `current`, `APP_RELEASE_ROOT`).
